            ...
            <div class="row" style="padding-top: 10px;" id="no_print1">
                <div class="form-group col-sm-4 col-xs-6">
                    <input type="hidden" class="form-control" 
                            placeholder="Reference" value="<?php echo $invoice_id; ?>" name="reference_invoice" id="reference_invoice">
                    <input type="text" class="form-control" id="address_invoice" 
                            placeholder="Address" value="<?= @$referance_info['address'] ?>" name="address_invoice">
                </div>
                <div class="form-group col-sm-4 col-xs-6">
                    <input type="text" class="form-control" id="project_name_invoice" 
                            placeholder="Project Name" value="<?= @$referance_info['project_name'] ?>" name="project_name_invoice">
                </div>
                <div class="form-group col-sm-4 col-xs-6">
                    <input type="text" class="form-control" id="po_no_invoice" 
                            placeholder="Indent/Requisition/PO" value="<?= @$referance_info['indent_no'] ?>" name="po_no_invoice">
                </div>
                <div class="form-group col-sm-4 col-xs-6">
                    <input type="text" class="form-control" id="mpr_invoice" 
                            placeholder="Machine Category/Model" value="<?= @$referance_info['mpr_no'] ?>" name="mpr_invoice">
                </div>
                <div class="form-group col-sm-4 col-xs-6">
                    <input type="text" class="form-control" id="customer_name" 
                            placeholder="Customer Name" required value="<?= @$referance_info['customer_name'] ?>" name="customer_name">
                </div>
                <div class="form-group col-sm-4 col-xs-6">
                    <input type="text" class="form-control" id="sales_type" 
                            placeholder="Sales Type" required value="<?= @$referance_info['status'] ?>" name="sales_type">
                </div>
                <div class="form-group col-sm-4 col-xs-6">
                <select name="currency" required id="currency" class="form-control">
                    <option value="">--Currency Select--</option>
                    <option <?php if($referance_info['currency']=="TK") echo "selected"; ?> value="TK">TK</option>
                    <option <?php if($referance_info['currency']=="USD") echo "selected"; ?> value="USD">USD</option>
                </select>
    ...
    <div id="print_div">
        <div class="box-body" style="width: 90%; color: black; padding-left: 30px">
            <p style="margin-left: 15px;">
                <a class="btn btn-flat btn-warning fa fa-print" href="<?php echo site_url("show_form/generate_bill_pdf/".$invoice_id) ?>" target="_blank" ></a>
            </p>
